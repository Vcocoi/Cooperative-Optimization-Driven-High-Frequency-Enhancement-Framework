...
class MFRFC(nn.Module):
    ...
    def dis_s(self, N, app_ord):  
        ...
        ec, vc = torch.linalg.eig(C2) # 计算奇异值（返回特征值，特征向量）
        es, vs = torch.linalg.eig(S2)
        ec = ec.real
        vc = vc.real
        es = es.real
        vs = vs.real
        qvc = torch.vstack((vc, torch.zeros([math.ceil(N/2-1), math.floor(N/2+1)]))) # 竖直方向上拼接
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.MFRFC = MFRFC(in_channels=C1, order=0.5)
        #self.SFC = SFC(in_channels=C1)

    def forward(self, x):
        out1 = self.conv1(x)

        out1 = self.MFRFC(out1) 
        #out1 = self.SFC(out1) 
