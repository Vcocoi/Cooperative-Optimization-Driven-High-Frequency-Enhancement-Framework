...
def parse_args():
    ...
    parser = ArgumentParser(description='HO-PAN')
    parser.add_argument('--config_file', type=str, default='configs/config_PAN.yaml', help='The global config file')
    ...
    parser.add_argument('--wandb', type=str2bool, default=True, help='Using wandb to visualize training loss')
    parser.add_argument('--device', type=int, default=0, choices=(0,1), help='Choose CUDA Devices 0 or 1')
...
def init_global_config(args):
    ...
    g_c.wandb = args.wandb
    g_c.device = f"cuda:{args.device}"
    return g_c
